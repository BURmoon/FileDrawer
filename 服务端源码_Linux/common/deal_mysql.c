...
void process_result_test(MYSQL *conn, MYSQL_RES *res_set)
{
	...
	// ysql_fetch_row 从使用 mysql_store_result 得到的结果结构中提取一行，并把它放到一个行结构中
    //当数据用完或发生错误时返回NULL.
...
int process_result_one(MYSQL *conn, char *sql_cmd, char *buf)
{
    ...
    //获取结果集的行数
    //mysql_num_rows 接受由 mysql_store_result 返回的结果结果集，并返回结果集中的行数
    ...
    //mysql_fetch_row 从结果结构中提取一行，并把它放到一个行结构中
    //当数据用完或发生错误时返回NULL
