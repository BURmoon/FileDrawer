...
int main()
{
    ...
    LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main():执行MD5秒传文件事件的CGI程序\n");
    ...
    while (FCGI_Accept() >= 0)
    {
        ...
        {
            char *out = return_status("003"); //token验证失败错误码
            if(out != NULL)
            {
                printf(out);
                free(out);
        ...
        {
            printf("No data from standard input\n");
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main():没有登陆用户信息\n");
        }
        else //获取登陆用户信息
        {
            char buf[4*1024] = {0};
            ...
            if(ret == 0)
            {
                LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main():读取用户登录信息失败，fread()返回值为 0\n");
                continue;
            }

            //解析json中信息{ user:xxx , token:xxx , md5:xxx , fileName:xxx }
            char user[128] = {0};
            char md5[256] = {0};
            char token[256] = {0};
            char filename[128] = {0};
            ret = get_md5_info(buf, user, token, md5, filename);//解析json中信息
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main():读取登陆用户信息 %s-%s\n", user, filename);
            if(ret != 0)
            {
                LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main():解析发送的客户端json包时发生错误\n");
                continue;
            }
            ...
            if(ret == 0)
            {
                LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main():token验证成功，进行秒传处理\n");
                deal_md5(user, md5, filename); //秒传处理
            }
            else
            {
                LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main():token验证失败\n");
...
void read_cfg()
{
    LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main() --> read_cfg()\n");
...
int get_md5_info(char *buf, char *user, char *token, char *md5, char *filename)
{
    LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main() --> get_md5_info()\n");
    ...
    if(NULL == root)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "read_cfg() --> cJSON_Parse()\n");
        ret = -1;
        goto END;
    }
    ...
    if(NULL == child1)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "read_cfg() --> cJSON_GetObjectItem(user)\n");
        ret = -1;
        goto END;
    }
    ...
    if(NULL == child2)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "read_cfg() --> cJSON_GetObjectItem(md5)\n");
        ret = -1;
        goto END;
    }
    ...
    if(NULL == child3)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "read_cfg() --> cJSON_GetObjectItem(fileName)\n");
        ret = -1;
        goto END;
    }
    ...
    if(NULL == child4)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "read_cfg() --> cJSON_GetObjectItem(token)\n");
        ret = -1;
        goto END;
    }
...
int deal_md5(char *user, char *md5, char *filename)
{
    LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "main() --> deal_md5()\n");
    ...
    if (conn == NULL)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "deal_md5() --> msql_conn()\n");
        ret = -1;
        goto END;
    }
    ...
    */
    ...
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "deal_md5() --> process_result_one() 服务器已拥有此文件\n");
        ...
        if(ret2 == 2) //如果有结果，说明用户已经保存此文件
        {
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "deal_md5() --> process_result_one() 用户已拥有此文件\n");
            ret = -2; //0秒传成功，-1出错，-2此用户已拥有此文件， -3秒传失败
            goto END;
        }

        //用户没有保存此文件
        //修改file_info中的count字段，令其+1 （count 文件引用计数）
        sprintf(sql_cmd, "update file_info set count = %d where md5 = '%s'", ++count, md5); //前置++
        if(mysql_query(conn, sql_cmd) != 0)
        {
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "deal_md5() --> mysql_query() error\n");
            ret = -1;
            goto END;
        }

        //在 user_file_list 用户文件列表插入一条数据
        //当前时间戳
        struct timeval tv;
        struct tm* ptm;
        char time_str[128];
        ...
        */
        sprintf(sql_cmd, "insert into user_file_list(user, md5, createtime, filename, pv) values ('%s', '%s', '%s', '%s', %d)", user, md5, time_str, filename, 0);
        if(mysql_query(conn, sql_cmd) != 0)
        {
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "deal_md5() --> mysql_query() error\n");
            ret = -1;
        ...
        //查询用户文件数量 user_file_count，更新文件计数令count +1
    ...
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "deal_md5() --> process_result_one() 秒传失败\n");
